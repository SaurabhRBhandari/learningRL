    ...
    def fit(self,batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            if not done:
                ...
                B=self.main_network(next_stat)
                Q=B.detach().to(device='cpu').numpy()
                Q_ = A.detach().to(device="cpu").numpy()
                Q_=Q_.ravel()
                target_Q = reward + self.gamma*Q_[np.argmax(Q)]
            else:
                continue
            
            Q_values=self.main_network(stat).to("cpu")
            loss=self.loss_fn(torch.max(Q_values),torch.tensor(np.float(target_Q)))
